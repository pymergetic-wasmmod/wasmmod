...
ports/cpython/
  ...
  finder.c         # pack path: POSIX + HTTP io + metal-cdn
...
| **Pack** | `wasm.path` / `sys.path` / metal-cdn → `import_pack` → `pack_bind` (wasm/aot/ELF) |
...
**Not yet:** Metal strong `pm_metal_wasm_io_*` (park). Stub is `ports/metal/`. Unix/CPython `https://` is the default io fill (mbedtls).

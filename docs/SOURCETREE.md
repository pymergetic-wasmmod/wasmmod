...
| 2026-08-12 | upywm host face restored at `ports/micropython/` (thin `modwasmmod.c` + `upy-host` staticlib): auto `install_hook`, `sys.modules` → `pm_wasmmod_registry_publish` presence sync, `load`/`call` over Rust registry+loader. No `host_slots`/`call0_py`. Pack finder (`loader/finder/`) still next |
